// Errors for ethnicity
function validateForm() {
  $('#error-form-group').removeClass('govuk-form-group--error');
  $('#error-message-radio, #error-summary').remove();

  var radioChecked = $('[name=victimEthnicity]').is(':checked');

  if (radioChecked) return true;

  var listHtml = '<li><a href="#error-message-radio">Select an ethnicity</a></li>';
  ...
  $('#radio-group').before('<p class="govuk-error-message" id="error-message-radio"><span class="govuk-visually-hidden">Error:</span> Select an ethnicity</p>');
