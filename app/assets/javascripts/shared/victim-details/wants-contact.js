// Errors for wants-contact
function validateForm() {
  $('#error-form-group').removeClass('govuk-form-group--error');
  $('#error-message-radio, #error-summary').remove();

  var radioChecked = $('[name=victimWantsContact]').is(':checked');

  if (radioChecked) return true;

  var listHtml = '<li><a href="#error-message-radio">Select yes if the victim wants to be contacted</a></li>';
  ...
  $('#radio-group').before('<p class="govuk-error-message" id="error-message-radio"><span class="govuk-visually-hidden">Error:</span> Select yes if the victim wants to be contacted</p>');
