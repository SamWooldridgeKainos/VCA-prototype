// Errors for preferred name
function validateForm() {
  $('#error-form-group-1').removeClass('govuk-form-group--error');
  $('#victim-preferred-name').removeClass('govuk-input--error');
  $('#error-message-1').remove();
  $('#error-summary').remove();

  var name = document.forms["myForm"]["victim-preferred-name"].value.trim();
  ...
  if (name === "") {
    ...
    $('#victim-preferred-name').before('<p class="govuk-error-message" id="error-message-1"><span class="govuk-visually-hidden">Error:</span> Enter the victim\'s preferred name</p>');
    $('#victim-preferred-name').addClass('govuk-input--error');
    $('#error-list-item-1').html('<a href="#error-message-1">Enter the victim\'s preferred name</a>');
