// Errors for power of attorney
function validateForm() {
  $('#error-form-group-1').removeClass('govuk-form-group--error');
  $('#power-of-attorney-name').removeClass('govuk-input--error');
  $('#error-message-1, #error-summary').remove();

  var name = document.forms["myForm"]["power-of-attorney-name"].value.trim();
  ...
  if (name === "") {
    ...
    $('#power-of-attorney-name').before('<p class="govuk-error-message" id="error-message-1"><span class="govuk-visually-hidden">Error:</span> Enter the power of attorney\'s name</p>');
    $('#power-of-attorney-name').addClass('govuk-input--error');
    $('#error-list-item-1').html('<a href="#error-message-1">Enter the power of attorney\'s name</a>');
