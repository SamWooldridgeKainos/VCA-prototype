...
function validateForm() {
    ...
    var taskAction = $('[name=taskAction]');

    if (!taskAction.is(':checked')) {
        // Error form group styling
        $('#error-form-group-1').addClass('govuk-form-group--error');

        // Error message
        $('#radio-group').before('<p id="error-message-1" class="govuk-error-message"><span class="govuk-visually-hidden">Error:</span> Select if you want to mark the task as complete</p>');

        // Error summary
        $('.govuk-heading-l').first().before(
            '<div id="error-summary" class="govuk-error-summary" aria-labelledby="error-summary-title" role="alert" tabindex="-1" data-module="govuk-error-summary"><h2 class="govuk-error-summary__title" id="error-summary-title">There is a problem</h2><div class="govuk-error-summary__body"><ul class="govuk-list govuk-error-summary__list"><li><a href="#error-message-1">Select if you want to mark the task as complete</a></li></ul></div></div>'
        );
        $('#error-summary').focus();
