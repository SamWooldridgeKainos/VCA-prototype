...
function validateForm() {
    ...
    $('#other-call-date').removeClass('govuk-input--error');
    $('#other-call-hour').removeClass('govuk-input--error');
    $('#other-call-minutes').removeClass('govuk-input--error');
    $('#other-individual-name').removeClass('govuk-input--error');
    $('#other-individual-role').removeClass('govuk-input--error');
    $('#other-purpose-of-communication').removeClass('govuk-textarea--error');
    ...
    var otherCallDate = $('#other-call-date').val();
    var otherCallHour = $('#other-call-hour').val();
    var otherCallMinutes = $('#other-call-minutes').val();
    var otherCallType = $('[name=otherCallType]');
    var otherIndividual = $('[name=otherIndividual]');
    var purposeOfCommunication = $('#other-purpose-of-communication').val();

    var errors = [];

    if (otherCallDate == '' || otherCallDate == null) {
        // Error form group styling
        $('#error-form-group-1').addClass('govuk-form-group--error');

        // Error input field styling
        $('#other-call-date').addClass('govuk-input--error');

        // Error message
        $('#other-call-date-hint').after('<p id="error-message-1" class="govuk-error-message"><span class="govuk-visually-hidden">Error:</span> Enter the date of call</p>');
    ...
    if (!otherCallType.is(':checked')) {
        ...
        // Error message
        $('#other-call-type-radios').before('<p id="error-message-3" class="govuk-error-message"><span class="govuk-visually-hidden">Error:</span> Select the direction of call</p>');
    ...
    if (purposeOfCommunication == '' || purposeOfCommunication == null) {
        ...
        $('#other-purpose-of-communication').addClass('govuk-textarea--error');

        // Error message
        $('#other-purpose-of-communication').before('<p id="error-message-5" class="govuk-error-message"><span class="govuk-visually-hidden">Error:</span> Enter the purpose of call</p>');

        errors.push('<li><a href="#error-message-5">Enter the purpose of call</a></li>');
    }
